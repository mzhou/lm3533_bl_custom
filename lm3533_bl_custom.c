...
{
	if (lm3533dev != NULL && lm3533dev->client != NULL)
		return &lm3533dev->client->dev;
	return NULL;
}
...
EXPORT_SYMBOL(lm3533_bl_custom_on_off);

static int saved_level_for_ssd2825;
int lm3533_bl_custom_onoff_ForHM(struct device *dev,int on_off)
{
	int ret = 0;
	struct i2c_client *client = to_i2c_client(dev);
	struct lm3533_device *lm3533_dev = i2c_get_clientdata(client);
	...
	if ((on_off == 1) && (lm3533_dev->bl_status == BL_OFF)) {
		...
		lm3533_backlight_off(client);
		ret = 1;
	} else if ((on_off == 0) && (lm3533_dev->bl_status == BL_OFF)) {
		ret = 0;
	} else if ((on_off == 1) && (lm3533_dev->bl_status == BL_ON)) {
		ret = 0;
	}

	return ret;
}
EXPORT_SYMBOL(lm3533_bl_custom_onoff_ForHM);
